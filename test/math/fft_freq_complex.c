...
int math_fft_freq_complex() {
	...
	for (size_t i = 0; i < BUFF_SIZE * 2; i += 2) {
		...
		original[i] = (float)rng / (float)UINT32_MAX;
		original[i + 1] = 0.0f;
		result[i] = original[i];
		result[i + 1] = 0.0f;
	...
	clod_fft(result, BUFF_SIZE, CLOD_FFT_INPUT * CLOD_FFT_TIME_COMPLEX + CLOD_FFT_OUTPUT * CLOD_FFT_FREQ_COMPLEX);
	clod_fft(result, BUFF_SIZE, CLOD_FFT_INPUT * CLOD_FFT_FREQ_COMPLEX + CLOD_FFT_OUTPUT * CLOD_FFT_TIME_COMPLEX);
	for (size_t i = 0; i < BUFF_SIZE * 2; i += 2) {
