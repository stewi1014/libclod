...
again:
	int64_t n = snd_pcm_readi(stream->in, dst->ptr + dst->len, (size_t)(dst->cap - dst->len) / sizeof(float));
	if (n < 0 && n != -EAGAIN) {
...
again:
	int64_t n = snd_pcm_writei(stream->out, src->ptr, (size_t)(src->len) / sizeof(float));
	if (n < 0 && n != -EAGAIN) {
		debug(CLOD_DEBUG, "Audio write error: %s", snd_strerror((int)n));
		n = snd_pcm_recover(stream->out, (int)n, 0);
		if (n < 0) {
			return -(int)n;
		}
...
int clod_audio_close(clod_stream *self) {
	...
	if (stream->in) {
		res = snd_pcm_close(stream->in);
		debug(CLOD_DEBUG, "Audio input close error: %s", snd_strerror(res));
	}
...
int clod_audio(clod_stream **stream_out, int flags, clod_allocator *allocator) {
	...
	if (flags & CLOD_AUDIO_IN) {
		...
		check_error(snd_pcm_hw_params_set_format(stream->in, hw_params, SND_PCM_FORMAT_FLOAT));
	...
	if (flags & CLOD_AUDIO_OUT) {
		...
		check_error(snd_pcm_hw_params_set_format(stream->out, hw_params, SND_PCM_FORMAT_FLOAT));
