...
static int clod_tree_print_recursive(
	...
	const int depth,
	const int max_depth,
	struct clod_tree_location *highlight
) {
	int res;
	#define print(fmt, ...) {\
	...
	if (depth_get(n) == 0) {
		if (n == tree->root_node) {
			print("%.*sTree %ptr (Leaf Root: %ptr, Keys: %u, Checksum: %Xu32, Shadowed: %s)\n",
				TABS, depth, (void*)tree, (void*)n, length_get(n),
				checksum_get(n), draft_get(n) ? "True" : "False");
		} else {
			print(
				"%.*sLeaf %ptr (Parent: %ptr, Next: %ptr, Prev: %ptr, Keys: %u, Checksum: %Xu32, Shadowed: %s)\n",
				TABS, depth, (void*)n, (void*)parent_get(n), (void*)leaf_next_get(n),
				(void*)leaf_prev_get(n), length_get(n), checksum_get(n), draft_get(n) ? "True" : "False"
			);
		}
		...
		if (n == tree->root_node) {
			print(
				"%.*sTree %ptr (Intl Root: %ptr, Keys: %u, Checksum: %Xu32, Shadowed: %s)\n",
				TABS, depth, (void*)tree, (void*)n, length_get(n), checksum_get(n), draft_get(n) ? "True" : "False"
			);
		} else {
			print(
				"%.*sIntl %ptr (Parent: %ptr, Keys: %u, Checksum: %Xu32, Shadowed: %s)\n",
				TABS, depth, (void*)n, (void*)parent_get(n), length_get(n), checksum_get(n), draft_get(n) ? "True" : "False"
			);
		}
...
int clod_tree_print(const struct clod_tree *tree, clod_stream *dst, const int max_depth) {
