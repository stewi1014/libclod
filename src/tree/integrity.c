...
CLOD_PURE
uint32_t checksum_compute(const struct clod_tree *tree, const node *n) {
	if (tree->disable_checksum) {
		return tree->checksum_seed &~ CHECKSUM_FLAGS;
	}
	...
	hash = clod_crc32_add(
		...
		n + CHECKSUM_OFFSET,
		depth_get(n) == 0 ?
			LEAF_SIZE - CHECKSUM_OFFSET :
			INTL_SIZE - CHECKSUM_OFFSET
	);
...
enum clod_tree_result before_read(const struct clod_tree *tree, node **ptr) {
	...
	if (tree->size) {
		assert_fatal(CLOD_DEBUG, n >= tree->root_node && n <= tree->root_node + tree->size,
			"Invalid pointer %ptr.", (void*)n);
	}
	...
	const uint32_t checksum = checksum_get(n);

	if (checksum & CHECKSUM_MODIFY_FLAG) {

		debug(CLOD_DEBUG, "Reading node %ptr, but it's currently being modified.", (void*)n);
		return false;

	}

	if (checksum != checksum_compute(tree, n)) {

		debug(
			CLOD_DEBUG,
			"Checksum mismatch for node %ptr. Stored checksum: %xu32, Calculated checksum: %xu32.",
			(void*)n, checksum, checksum_compute(tree, n)
		);
...
enum clod_tree_result before_modify(const struct clod_tree *tree, node **ptr) {
	...
	assert_fatal(
		CLOD_DEBUG,
		checksum_get(n) == checksum_compute(tree, n),
		"Checksum mismatch for node %ptr. Stored checksum: %xu32, Calculated checksum: %xu32.\n"
		"Either an illegal modification was made, or before_read was never called on this node.",
		(void*)n, checksum_compute(tree, n), checksum_get(n)
	);

	if (tree->shadow && !draft_get(n)) {
		if (depth_get(n)) {
			node *draft = tree->allocator.allocate(INTL_SIZE, tree->allocator.self);
			if (!draft) {
			...
			draft_set(n, draft);
			copy(draft, n, INTL_SIZE);
			*ptr = draft;
			n = draft;
		} else {
			node *draft = tree->allocator.allocate(LEAF_SIZE, tree->allocator.self);
			if (!draft) {
			...
			draft_set(n, draft);
			copy(draft, n, LEAF_SIZE);
			*ptr = draft;
			n = draft;
		}
	}

	checksum_set(n, CHECKSUM_MODIFY_FLAG);
	return true;
}
...
void after_modify(const struct clod_tree *tree, node *n) {
	...
	assert_fatal(
		CLOD_DEBUG,
		!tree->shadow || draft_get(n) == n,
		"Only draft nodes may be modified, but shadow node %ptr was passed to after_modify.",
		(void*)n
	);

	assert_fatal(CLOD_DEBUG, checksum_get(n) == 0 || checksum_get(n) == CHECKSUM_MODIFY_FLAG,
		"before_modify was not called on node %ptr.", (void*)n);

	checksum_set(n, checksum_compute(tree, n));
}
...
bool leaf_validate(
	...
	if (parent_get(n) && length_get(n) < tree->leaf_cap / 2) {
		debug(CLOD_DEBUG, "Leaf node %ptr has %u keys which is less than half the capacity (%u out of %u total) required for a balanced tree.",
			(void*)n, length_get(n), tree->leaf_cap / 2, tree->leaf_cap);

		ok = false;
	}

	if (length_get(n) > tree->leaf_cap) {
		debug(CLOD_DEBUG, "Leaf node %ptr has %u keys, which is more than the capacity (%u).",
			(void*)n, length_get(n), tree->leaf_cap);
...
bool intl_validate(
	...
	if (parent_get(n) && length_get(n) < tree->intl_cap / 2) {
		debug(CLOD_DEBUG, "Internal node %ptr has %u keys which is less than half the capacity (%u out of %u total) required for a balanced tree.",
			(void*)n, length_get(n), tree->intl_cap / 2, tree->intl_cap);

		ok = false;
	}

	if (length_get(n) > tree->intl_cap) {
		debug(CLOD_DEBUG, "Internal node %ptr has %u keys, which is more than the capacity (%u).",
			(void*)n, length_get(n), tree->intl_cap);
...
bool clod_tree_validate(const struct clod_tree *tree) {
	...
	#if CLOD_DEBUG
	if (!ok) {
		clod_tree_print(tree, clod_stderr, 5);
	}
	#endif
